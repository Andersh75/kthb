...
async _selectedmuniChanged() {
    ...
    if (names.length) {
        let news = await this.grabMunis(selectedNames, newIds, newArr);
...
async grabMuni(name, skip, limit, max, newIds, newArr) {
    ...
    if (max > 0) {
        return this.grabMuni(name, skip + 1000, 1000, max, newIds, newArr);
    } else {
        return {'ids': newIds, 'arr': newArr};
    }
};

async grabMunis(names, newIds, newArr) {
    let news = await this.grabMuni(names[0].key, 0, 1000, names[0].value, newIds, newArr);
    names.shift();

    if (names.length > 0) {
        return this.grabMunis(names, news.ids, news.arr);
    } else {
