...
async getMunicipalities() {
    const res = await this.db.query('my_index8/by_municipality', {
        startkey: 'A',
        reduce: true,
        group: true,
    });
    ...
    const fillTable = (e) => {
        this.municipalitytableitems = e.data;
    };
    this.myWorker.postMessage(res);
    this.myWorker.onmessage = fillTable;
};

async getBrokers() {
    const res = await this.db.query('my_index8/by_name', {
        startkey: 'A',
        reduce: true,
        group: true,
    });
...
async getBuildyear() {
    const res = await this.db.query('my_index8/by_buildyear', {
        startkey: 'A',
        reduce: true,
        group: true,
    });
    ...
    const fillTable = (e) => {
        this.buildyeartableitems = e.data;
    };
    this.myWorker.postMessage(res);
    this.myWorker.onmessage = fillTable;
};
...
async getContractdate() {
    const res = await this.db.query('my_index8/by_contractdate', {
        startkey: '1',
        reduce: true,
        group: true,
    });
    ...
    const fillTable = (e) => {
        this.contractdatetableitems = e.data;
    };
    this.myWorker.postMessage(res);
    this.myWorker.onmessage = fillTable;
};
...
_selectedmuniChanged2() {
    ...
    if (names.length) {
        ...
        async function grabIt(skip, limit, max) {
            ...
            let myArr = this._selectedColumnChanged(this.arrIdsMuniOld, this.arrMunicipalityOld, result);
            this.arrIdsMuniOld = myArr[1];
            this.arrMunicipalityOld = myArr[0];
