...
//     _municipalit_contractdatetableitemsChangedytableitemsChanged() {
        ...
        <div class="grid-12 container">
            ...
            <div class="col1span12 grid-12 main">
               ...
               <whcg-lit-grid slot="input2" items="{{municipalitytableitems}}" selecteditems="{{selectedmuni}}">
                ...
                <whcg-lit-grid slot="input3" items="{{buildyeartableitems}}">
                    <vaadin-grid-selection-column auto-select></vaadin-grid-selection-column>
                    <vaadin-grid-column width="140px" flex-grow="1" flex-shrink="1" resizable>
               ...
               <whcg-lit-grid slot="input4" items="{{contractdatetableitems}}">
                    <vaadin-grid-selection-column auto-select></vaadin-grid-selection-column>
                    <vaadin-grid-column width="140px" flex-grow="1" flex-shrink="1" resizable>
...
async getMunicipalities(skip, limit, max) {
    ...
    const fillTable = (e) => {
        ...
        if (e.data.rows.length > 0 && max > 0) {
            this.getMunicipalities(skip + 20, 20, max - 20);
        } 
    };
...
async getBrokers(skip, limit, max) {
    ...
    const fillTable = (e) => {
        ...
        if (e.data.rows.length > 0 && max > 0) {
            this.getBrokers(skip + 20, 20, max - 20);
        } 
    };
...
async getBuildyear(skip, limit, max) {
    ...
    const fillTable = (e) => {
        ...
        if (e.data.rows.length > 0 && max > 0) {
            this.getBuildyear(skip + 20, 20, max - 20);
        } 
    };
...
async getContractdate(skip, limit, max) {
    ...
    const fillTable = (e) => {
        ...
        if (e.data.rows.length > 0 && max > 0) {
            this.getContractdate(skip + 20, 20, max - 20);
        } 
    };
...
async _selectedmuniChanged() {
    ...
    console.log('_selectedmuniChanged');
    console.log(this.selectedmuni);

    if (names.length) {
        this.grabMunis(names, this.arrIdsMuniOld, this.arrMunicipalityOld)
    } else {
...
async grabMuni(name, skip, limit, max, oldIds, oldArr) {
    console.log('name!');
    console.log(name);
    let result = await this.db.find({
        selector: { 
           municipality: { $eq: name },
        },
        limit: limit,
        skip: skip
    });

    console.log('muni result');
    console.log(result);

    let myArr = await this._selectedColumnChanged(oldIds, oldArr, result.docs);
    ...
    if (max > 0) {
        this.grabMuni(name, skip + 20, 20, max, oldIds, oldArr);
    } else {
        return {'ids': oldIds, 'arr': oldArr};
    }
};

async grabMunis(names, oldIds, oldArr) {
    console.log('names');
    console.log(names);
    console.log('oldIds');
    console.log(oldIds);
    console.log('oldArr');
    console.log(oldArr);
    let old = await this.grabMuni(names[0], 0, 20, 1000, oldIds, oldArr);
    names.shift();

    if (names.length > 0) {
        this.grabMunis(names, old.ids, old.arr);
    } else {
...
_selectedColumnChanged(selectedColumnsIdOld, selectedColumnsArrOld, res) {
    ...
    let arrdiff = this.arr_diff(newArrIdsName, selectedColumnsIdOld);
    ...
    return [selectedColumnsArrOld, selectedColumnsIdOld];
  ...
  connectedCallback() {
      ...
      this.getMunicipalities(0, 20, 100);
    //   this.getBrokers(0, 20, 100);
    //   this.getBuildyear(0, 20, 100);
    //   this.getContractdate(0, 20, 100);
